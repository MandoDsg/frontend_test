...
import 'package:frontend/views/movilidad_integrada/maps/cablebus_l1_map.dart';

class SubCablebusL1 extends StatefulWidget {
  const SubCablebusL1({Key? key}) : super(key: key);
        ...
        appBar: AppBar(
            ...
            IconButton(
                onPressed: () {
                  Navigator.push(
                      context,
                      MaterialPageRoute(
                          builder: (context) => const CablebusL1Map()));
                },
          ...
          bottom: PreferredSize(
            ...
            child: Row(
              ...
              children: [
                invertListOrder
                    ? const Text("Cuautepec/Tlalpexco",
                        style: TextStyle(color: Colors.white))
                    : const Text("Indios Verdes",
                        style: TextStyle(color: Colors.white)),
                const Icon(Icons.swap_horiz, color: Colors.white),
                invertListOrder
                    ? const Text("Indios Verdes",
                        style: TextStyle(color: Colors.white))
                    : const Text("Tlalpexco/Cuautepec",
                        style: TextStyle(color: Colors.white)),
              ],
            ),
          ),
...
class StationsApi {
  static Future<List<StationData>> getUsersLocally(BuildContext context) async {
    ...
    final data =
        await assetBundle.loadString('assets/data/Cablebus/linea_1.json');
